...
fn import_call_implementation(mut env: FunctionEnvMut<HostEnv>, table_ptr: i32, extra_table_ptr: i32) -> Result<i32, RuntimeError> {
    ...
    let is_valid_pk = amadeus_utils::bls12_381::validate_public_key(contract.as_slice()).is_ok();
    let result = if is_valid_pk {
...
fn import_storage_kv_increment_implementation(mut env: FunctionEnvMut<HostEnv>, key_ptr: i32, key_len: i32, val_ptr: i32, val_len: i32) -> Result<i32, RuntimeError> {
    ...
    let new_value = kv_increment(applyenv, &key, value_int128).unwrap_or_else(|_| panic_any("kv_increment_failed")).to_string();
...
fn inject_env_data(view: &MemoryView, env: &ApplyEnv) {
    ...
    let signer_bytes: &[u8] = env.caller_env.entry_signer.as_ref();
    let prev_hash_bytes: &[u8] = env.caller_env.entry_prev_hash.as_ref();
    let dr_bytes: &[u8] = env.caller_env.entry_dr.as_ref();
    let tx_signer_bytes: &[u8] = env.caller_env.tx_signer.as_ref();

    w(2_100, &(signer_bytes.len() as u32).to_le_bytes());
    w(2_104, signer_bytes);
    w(2_200, &(prev_hash_bytes.len() as u32).to_le_bytes());
    w(2_204, prev_hash_bytes);
    w(2_300, &(env.caller_env.entry_vr.len() as u32).to_le_bytes());
    w(2_304, &env.caller_env.entry_vr);
    w(2_400, &(dr_bytes.len() as u32).to_le_bytes());
    w(2_404, dr_bytes);

    w(3_000, &env.caller_env.tx_nonce.to_le_bytes());
    w(3_100, &(tx_signer_bytes.len() as u32).to_le_bytes());
    w(3_104, tx_signer_bytes);
