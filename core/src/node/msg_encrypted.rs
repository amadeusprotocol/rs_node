...
/// EncryptedMessage format compatible with Elixir implementation
/// Uses BLS-compatible shared secrets and AES-256-GCM encryption
#[derive(Debug, Clone)]
...
impl EncryptedMessage {
    ...
    /// Returns a vector of EncryptedMessage instances (one per shard)
    pub fn encrypt(
        sender_pk: &[u8; 48],
        shared_secret: &[u8],
        plaintext: &[u8],
        ...
        // Split ciphertext and tag for Elixir compatibility (IV + tag + ciphertext)
        ...
        if encrypted_payload.len() < 1300 {
            ...
            let data_shards = encrypted_payload.len().div_ceil(1024);
    ...
    /// Decrypt a single EncryptedMessage (includes decompression for direct use)
...
/// Reassembler for EncryptedMessage shards - matches ReedSolomonReassembler interface exactly
pub struct EncryptedMessageReassembler {
    reorg: RwLock<HashMap<EncryptedReassemblyKey, EncryptedEntryState>>,
}

#[derive(Clone, Debug, Eq)]
struct EncryptedReassemblyKey {
    pk: [u8; 48],
    ts_nano: u64,
    shard_total: u16,
    original_size: u32,
...
#[derive(Debug)]
enum EncryptedEntryState {
    Collecting(HashMap<u16, Vec<u8>>), // shard_index -> encrypted shard data
    Spent,
}

impl Default for EncryptedMessageReassembler {
    fn default() -> Self {
        Self::new()
    }
}
...
impl EncryptedMessageReassembler {
        ...
        let mut maybe_shards: Option<Vec<(usize, Vec<u8>)>> = None;
        {
            ...
            match map.entry(key.clone()) {
                Entry::Vacant(v) => {
                    ...
                    v.insert(EncryptedEntryState::Collecting(state_map));
                }
                Entry::Occupied(mut occ) => {
                    match occ.get_mut() {
                        EncryptedEntryState::Spent => {
                            // nothing to do
                        }
    ...
    pub fn build_shards(
        ...
        let encrypted_messages = EncryptedMessage::encrypt(&sender_pk, &shared_secret, payload, version)?;
...
mod tests {
    ...
    fn test_encrypted_message_round_trip() {
        ...
        let deserialized = EncryptedMessage::try_from(serialized.as_slice()).expect("deserialization should succeed");
    ...
    fn test_elixir_compatible_64_byte_keys() {
        ...
        println!("✓ EncryptedMessage 64-byte key compatibility test passed");
    }
    ...
    async fn test_encrypted_message_reassembler() {
        ...
        let encrypted_messages = EncryptedMessage::encrypt(&pk_alice, &shared_secret, test_message, version)
            .expect("encryption should succeed");

        let reassembler = EncryptedMessageReassembler::new();
    ...
    fn test_build_shards() {
        ...
        // Build shards
        let shards = EncryptedMessageReassembler::build_shards(&config, test_payload, &target_pk)
            .expect("build_shards should succeed");

        assert!(!shards.is_empty(), "Should create at least one shard");

        // Each shard should be valid EncryptedMessage binary
        for shard in &shards {
    ...
    fn test_build_broadcast_shards() {
        ...
        // Build broadcast shards (uses own key as target)
        let shards = EncryptedMessageReassembler::build_broadcast_shards(&config, test_payload)
            .expect("build_broadcast_shards should succeed");
